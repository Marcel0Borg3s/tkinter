...
Label(frame, text="Qual é o seu peso? (kg) ").grid(column=1, row=2)
# Adicionando o input:
peso_kg = Entry(frame)
peso_kg.grid(column=2, row=2) 

Label(frame, text="Qual é o sua altura?").grid(column=1, sticky="w", row=3)
# Adicionando o input:
altura_cm = Entry(frame)
altura_cm.grid(column=2, row=3)

Label(frame, text="Qual é sua idade? ").grid(column=1, sticky="w", row=4)
# Adicionando o input:
idade = Entry(frame)
idade.grid(column=2, row=4)

# Botão para criar o calculo
Button(frame, text="Calcular", pady=5, command=calcular).grid(column=1, row=6, columnspan=2)

# Rótulo para exibir o resultado
resultado_label = Label(frame, text="", font=("Arial", 12), pady=10)
resultado_label.grid(column=1, row=5, columnspan=2)
